...
obvious — Fivetran (sync) and GitHub (action). Hit all four criteria: Technological Implementation,
...
2. **Live sync (0:20–0:45)** — Click **Sync Now**. The **MCP Activity Log** lights up:
   `[Fivetran MCP] → sync_connector(github) → success`. Call out the mandatory partner integration.
3. **The bridge — the money shot (0:45–1:40)** — Show the **Proposal Queue**: the agent read a Slack
   thread ("we should add rate-limiting to the auth endpoint") and drafted a GitHub issue with a
   suggested assignee (the *least-loaded* suitable dev). Click **Approve** →
   `[GitHub MCP] → create_issue → success`, and the real issue opens in `stealth-labs-platform`, linked back to the
   thread. Emphasize: agent proposes, human approves, GitHub MCP acts.
...
5. **Chat with visible reasoning (2:15–2:50)** — Ask: "Why did the auth feature take so long?" The
   agent streams thinking + tool calls (BigQuery + GitHub) and answers with evidence (PR + ticket +
   meeting load).
...
- [ ] One genuinely "stuck" PR and one overloaded dev in the data
- [ ] At least one pending issue proposal queued and ready to approve live
- [ ] GitHub repo (`stealth-labs-platform`) reachable; the create_issue call works end-to-end
- [ ] Hosted Cloud Run URL loads cleanly
- [ ] MCP log scrolls visibly during sync + approval + chat (both Fivetran + GitHub MCP appear)
